  ...
  if (client.connect(pi_ip, pi_port)) {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (command == "SNAP") {
          Serial.println("SNAP command received! Adjusting exposure...");
          
          // Discard first few frames for exposure/WB
          for (int i = 0; i < 5; i++) {
          ...
          break; // Close connection to signal completion
        }
      }
    }
    
    client.stop();
    Serial.println("Connection closed. Reconnecting for next round...");
    delay(500); 
