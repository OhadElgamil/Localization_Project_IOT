...
void sendPhotoTCP() {
  ...
  // Discard first few frames so sensor can adjust exposure/WB
  for (int i = 0; i < 5; i++) {
  ...
  if (client.connect(pi_ip, pi_port)) {
    Serial.println("Connected to Pi. Sending photo...");
    client.write(fb->buf, fb->len);
    client.stop();
    Serial.println("Photo sent successfully!");
  } else {
    Serial.println("Connection to Pi failed. Is the Python script running?");
  }
