"""Cross-cycle smoothing for localization.estimate() output.

Every pipeline cycle solves an independent pose from scratch (see
localization.py's module docstring) -- correct, but real corner-pixel noise
means consecutive cycles' raw positions jitter even when the rig is
stationary. PoseSmoother folds each new pose into a running estimate with an
exponential moving average, scaled by that cycle's own `confidence` so a
shaky, low-confidence read nudges the output less than a sharp one.
